...
import { getMentoringList } from './apis/getMentorList';
...
function Home() {
  ...
  useEffect(() => {
    const fetchMentorData = async () => {
      try {
        const convertedSelectedSpecialties: Record<string, string> = {};
        selectedSpecialties.forEach((specialty, index) => {
          convertedSelectedSpecialties[`categoryTitle${index + 1}`] = specialty;
        });

        const data = await getMentoringList({
          params: convertedSelectedSpecialties,
        });
