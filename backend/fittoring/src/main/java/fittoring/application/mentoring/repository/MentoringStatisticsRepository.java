...
public interface MentoringStatisticsRepository extends ListCrudRepository<MentoringStatistics, Long> {
    ...
    @Query(value = "SELECT * FROM mentoring_statistics WHERE mentoring_id = :mentoringId AND is_deleted = true;", nativeQuery = true)
    MentoringStatistics findDeletedByMentoringId(@Param("mentoringId") Long mentoringId);
